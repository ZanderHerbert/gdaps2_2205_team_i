...
namespace Roboquatic
{
    ...
    class Hud
    {
        ...
        public void Draw(SpriteBatch sb, Player player, int screenHeight, int checkpointsCrossed, double percent)
        {
            ...
            {
                sb.Draw(healthSheet, new Rectangle(113 + i * 21, 20, 21, 54), new Rectangle(155, 0, 35, 90), Color.White);
            }
            ...
            if(player.Health == 1)
            {
                sb.Draw(healthSheet, new Rectangle(32, 41, 15, 18), new Rectangle(20, 95, 25, 30), Color.White);
            }
            else if(player.Health == 2)
            {
                sb.Draw(healthSheet, new Rectangle(32, 41, 36, 18), new Rectangle(20, 125, 60, 30), Color.White);
            }
            else if (player.Health == 3)
            {
                sb.Draw(healthSheet, new Rectangle(32, 41, 57, 18), new Rectangle(20, 160, 95, 30), Color.White);
            }
            else if (player.Health == 4)
            {
                sb.Draw(healthSheet, new Rectangle(32, 41, 78, 18), new Rectangle(20, 195, 130, 30), Color.White);
            }
            else if (player.Health == 5)
            {
                sb.Draw(healthSheet, new Rectangle(32, 41, 99, 18), new Rectangle(20, 230, 165, 30), Color.White);
            }
            else if (player.Health == 6)
            {
                sb.Draw(healthSheet, new Rectangle(32, 41, 120, 18), new Rectangle(20, 265, 200, 30), Color.White);
            }
            else
            {
                sb.Draw(healthSheet, new Rectangle(32, 41, 120, 18), new Rectangle(20, 300, 200, 30), Color.White);
                rightHealthOffset = 0;
                for(int i = 0; i < player.Health - 7; i++)
                {
                    sb.Draw(healthSheet, new Rectangle(155 + i * 21, 41, 18, 18), new Rectangle(50, 300, 30, 30), Color.White);
                    rightHealthOffset = 21 * (i + 1);
                }
                if(player.Health == player.MaxHP)
                {
                    sb.Draw(healthSheet, new Rectangle(155 + rightHealthOffset, 41, 18, 18), new Rectangle(225, 300, 30, 30), Color.White);
                }
                else
                {
                    sb.Draw(healthSheet, new Rectangle(155 + rightHealthOffset, 41, 18, 18), new Rectangle(50, 300, 30, 30), Color.White);
                }
            }
            ...
            sb.Draw(checkpointFlag, new Rectangle(588, screenHeight - 50, 32, 32), Color.White);
            if (checkpointsCrossed == 3)
            {
                sb.Draw(progressBarFiller, new Rectangle(603, screenHeight - 33, 98, 10), new Rectangle(0, 0, 65, 10), Color.Red);
                sb.Draw(playerTexture, new Rectangle(608, screenHeight - 43, 32, 32), Color.White);
                sb.Draw(bossTexture, new Rectangle(640, screenHeight - 43, 64, 32), null, Color.White, 0, Vector2.Zero, SpriteEffects.FlipHorizontally, 0);
            }
        }
    }
}
